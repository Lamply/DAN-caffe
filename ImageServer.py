...
def landmarksErrorNorm(landmark1, landmarks2):

    # transformedLandmarks = landmark1 + bias1

    meanError = np.mean(np.sqrt(np.sum((landmark1 - landmarks2) ** 2, axis=2)), axis=1)
    eyeDist = np.linalg.norm(np.mean(landmarks2[:, 36:42], axis=1) - np.mean(landmarks2[:, 42:48], axis=1), axis=1)

    error = np.mean(meanError / eyeDist)

    return error


def landmarksErrorNorm(landmark1, landmarks2, eyeDist):

    # transformedLandmarks = np.reshape(landmark1[:, :136], (-1, 68, 2))
    # transformedLandmarks = landmark1 + bias1
...
def landmarkPairErrorNorm(landmark1, bias1, landmarks2, eyeDist):

    transformedLandmarks = np.reshape(landmark1[:136], (68, 2))
    transformedLandmarks = transformedLandmarks + bias1

    meanError = np.mean(np.sqrt(np.sum((transformedLandmarks - landmarks2) ** 2, axis=1)))

    # eyeDist = np.linalg.norm(np.mean(landmarks2[36:42], axis=0) - np.mean(landmarks2[42:48], axis=0))

    error = meanError / eyeDist
...
class ImageServer(object):
    ...
    def setEyeDist(self):
        self.eyeDist = np.linalg.norm(np.mean(self.gtLandmarks[:, 36:42], axis=1) - np.mean(self.gtLandmarks[:, 42:48], axis=1), axis=1)


    def To_h5py(self, name):
        # self.setEyeDist()
        with h5py.File(os.getcwd() + '/' + name + '.h5', 'w') as f:
            f['data'] = self.imgs.astype(np.float32)
            f['initlandmark'] = self.initLandmarks.astype(np.float32)
            f['gtlandmark'] = self.gtLandmarks.astype(np.float32)
            f['eyedist'] = self.eyeDist.astype(np.float32)
        with open(os.getcwd() + '/' + name + '.txt', 'w') as f:
            f.write(os.getcwd() + '/' + name + '.h5\n')
