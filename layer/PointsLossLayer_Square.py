...
class PointsLossLayer(caffe.Layer):
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        # bottom[0].diff[...] = self.diff / (board_l2norm * bottom[0].shape[0] * 68 * self.board_eyedist)
            # bottom[0].diff[...] = self.diff / (board_eyedist * bottom[0].shape[0])
            # bottom[0].diff[...] = self.diff * top[0].diff[0] / (bottom[0].shape[0] * board_eyedist)
